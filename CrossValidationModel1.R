# Cross-validate results by setting aside 20% of CAMS data, building one-person household
# and two-person household regression models using remaining 80% of CAMS data (training set).
# Predict consumption for the households in the 20% of CAMS data set aside as the validation set.
# Compare model predictions to actual data with 100 cross-validations of the validation set.
# NOTE: For CAMS data that has associated wealth and income in HRS, transfer this HRS data to CAMS
# data with the same HHID.

## Loop through multiple cross-validation scenarios

n <- 100 # cross-validations to run
crossValOnePerson <- matrix(0,112,n)
crossValTwoPerson <- matrix(0,244,n)
...
cat("\nOverpredicts One-person Household consumption ",round(100 * (sum(crossValM11P$Error > 0) / length(crossValM11P$Error)),2)," percent of households.")
cat("\nOverpredicts Two-person Household consumption ",round(100  * (sum(crossValM12P$Error > 0) / length(crossValM12P$Error)),2)," percent of households.")
cat("\nOverpredicts All Household consumption ",round(100 * ((sum(crossValM11P$Error > 0) + sum(crossValM12P$Error > 0)) / (length(crossValM11P$Error) + length(crossValM12P$Error))),2)," percent of households.")
...
# graph density of all n scenarios for one-person households
# cat("\nRemoved ",sum(crossValOnePerson > 5)," outliers with greater than 500% error from ",length(crossValOnePerson)," samples.")
# crossValTwoPerson[crossValTwoPerson < -100000 ] <- 0
plot(density(crossValTwoPerson[,1]),main="Density Plots for 2-Person Prediction Errors",ylim=c(0,.00005),xlim=c(-100000,50000),xlab="Prediction Error ($)")
for (i in 2:n){
