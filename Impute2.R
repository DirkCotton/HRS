...
# create columns =1 if retirement in that wave, else zero  
  ...
  fit <- lm(cams1Pgtw$cndurWOR ~ cams1Pgtw$ssIncAtRetire1 + cams1Pgtw$portfolioAssetsTotal + cams1Pgtw$pensionIncome + cams1Pgtw$ageAtRetirement1 + cams1Pgtw$male + cams1Pgtw$earliestRetire +  cams1Pgtw$EarliestSquared  -1)
  
  cat("\n\n")
  print(summary(fit))
  
  cams1Pgtw$cndurExpected <- (fit$coefficients[5] * cams1Pgtw$male + fit$coefficients[1] * cams1Pgtw$ssIncAtRetire1 + fit$coefficients[2] * cams1Pgtw$portfolioAssetsTotal + fit$coefficients[3] * cams1Pgtw$pensionIncome + fit$coefficients[4] * cams1Pgtw$ageAtRetirement1 + fit$coefficients[6] * cams1Pgtw$earliestRetire + fit$coefficients[7] * cams1Pgtw$EarliestSquared)
  ...
  hrs$cndurImputed[hrs$earliestRetire >= 5 & hrs$persons == 1] <- (fit$coefficients[1] * hrs$d5[hrs$earliestRetire >= 5 & hrs$persons == 1] + fit$coefficients[2] * hrs$d6[hrs$earliestRetire >= 5 & hrs$persons == 1] +fit$coefficients[3] * hrs$d7[hrs$earliestRetire >= 5 & hrs$persons == 1] + fit$coefficients[4] * hrs$d8[hrs$earliestRetire >= 5 & hrs$persons == 1] + fit$coefficients[5] * hrs$d9[hrs$earliestRetire >= 5 & hrs$persons == 1] + fit$coefficients[6] * hrs$d10[hrs$earliestRetire >= 5 & hrs$persons == 1] + fit$coefficients[7] * hrs$d11[hrs$earliestRetire >= 5 & hrs$persons == 1] + fit$coefficients[8] * hrs$d12[hrs$earliestRetire >= 5 & hrs$persons == 1] + fit$coefficients[9] * hrs$male[hrs$earliestRetire >= 5 & hrs$persons == 1] + fit$coefficients[10] * hrs$ssIncAtRetire1[hrs$earliestRetire >= 5 & hrs$persons == 1] / hrs$gtw[hrs$earliestRetire >= 5 & hrs$persons == 1] + fit$coefficients[11] * hrs$portfolioAssetsTotal[hrs$earliestRetire >= 5 & hrs$persons == 1] / hrs$gtw[hrs$earliestRetire >= 5 & hrs$persons == 1] + fit$coefficients[12] * hrs$pensionIncome[hrs$earliestRetire >= 5 & hrs$persons == 1] / hrs$gtw[hrs$earliestRetire >= 5 & hrs$persons == 1] + fit$coefficients[13] * hrs$ageAtRetirement1[hrs$earliestRetire >= 5 & hrs$persons == 1]) 
...
#########################################  
  ...
  fit2 <- lm(cams2Pgtw$cndurWOR ~ cams2Pgtw$ssIncAtRetire1 + cams2Pgtw$ssIncAtRetire2 + cams2Pgtw$portfolioAssetsTotal + cams2Pgtw$pensionIncome + cams2Pgtw$ageAtRetirement1 + cams2Pgtw$ageAtRetirement2 + cams2Pgtw$earliestRetire + cams2Pgtw$EarliestSquared -1) # "-1" tells lm to ignore the y-intercept. d-columns will be the y-intercept.
  
  cat("\n\n")
  print(summary(fit2))
  

  cams2Pgtw$cndurExpected <- (fit2$coefficients[1] * cams2Pgtw$ssIncAtRetire1 + fit2$coefficients[2] * cams2Pgtw$ssIncAtRetire2 + fit2$coefficients[3] * cams2Pgtw$portfolioAssetsTotal + fit2$coefficients[4] * cams2Pgtw$pensionIncome + fit2$coefficients[5] * cams2Pgtw$ageAtRetirement1 + fit2$coefficients[6] * cams2Pgtw$ageAtRetirement2 + fit2$coefficients[7] * cams2Pgtw$earliestRetire + fit2$coefficients[8] * cams2Pgtw$EarliestSquared)
  ...
  hrs$cndurImputed[hrs$earliestRetire >= 5 & hrs$persons == 2] <- (fit2$coefficients[1] * hrs$d5[hrs$earliestRetire >= 5 & hrs$persons == 2] + fit2$coefficients[2] * hrs$d6[hrs$earliestRetire >= 5 & hrs$persons == 2] + fit2$coefficients[3] * hrs$d7[hrs$earliestRetire >= 5 & hrs$persons == 2] + fit2$coefficients[4] * hrs$d8[hrs$earliestRetire >= 5 & hrs$persons == 2] + fit2$coefficients[5] * hrs$d9[hrs$earliestRetire >= 5 & hrs$persons == 2] + fit2$coefficients[6] * hrs$d10[hrs$earliestRetire >= 5 & hrs$persons == 2] + fit2$coefficients[7] * hrs$d11[hrs$earliestRetire >= 5 & hrs$persons == 2] + fit2$coefficients[8] * hrs$d12[hrs$earliestRetire >= 5 & hrs$persons == 2] + fit2$coefficients[9] * hrs$ssIncAtRetire1[hrs$earliestRetire >= 5 & hrs$persons == 2] / hrs$gtw[hrs$earliestRetire >= 5 & hrs$persons == 2] + fit2$coefficients[10] * hrs$ssIncAtRetire2[hrs$earliestRetire >= 5 & hrs$persons == 2] / hrs$gtw[hrs$earliestRetire >= 5 & hrs$persons == 2] + fit2$coefficients[11] * hrs$portfolioAssetsTotal[hrs$earliestRetire >= 5 & hrs$persons == 2] / hrs$gtw[hrs$earliestRetire >= 5 & hrs$persons == 2] + fit2$coefficients[12] * hrs$pensionIncome[hrs$earliestRetire >= 5 & hrs$persons == 2] / hrs$gtw[hrs$earliestRetire >= 5 & hrs$persons == 2] + fit2$coefficients[13] * hrs$ageAtRetirement1[hrs$earliestRetire >= 5 & hrs$persons == 2] + fit2$coefficients[14] * hrs$ageAtRetirement2[hrs$earliestRetire >= 5 & hrs$persons == 2]) 
  ...
  hrs$cndurImputed[hrs$earliestRetire <= 4 & hrs$persons == 1] <- (fit3$coefficients[1] + fit3$coefficients[2] * hrs$male[hrs$earliestRetire <= 4 & hrs$persons == 1] / hrs$gtw[hrs$earliestRetire <= 4 & hrs$persons == 1] + fit3$coefficients[3] * hrs$ssIncAtRetire1[hrs$earliestRetire <= 4 & hrs$persons == 1] / hrs$gtw[hrs$earliestRetire <= 4 & hrs$persons == 1] + fit3$coefficients[4] * hrs$portfolioAssetsTotal[hrs$earliestRetire <= 4 & hrs$persons == 1] / hrs$gtw[hrs$earliestRetire <= 4 & hrs$persons == 1] + fit3$coefficients[5] * hrs$pensionIncome[hrs$earliestRetire <= 4 & hrs$persons == 1] / hrs$gtw[hrs$earliestRetire <= 4 & hrs$persons == 1] + fit3$coefficients[6] * hrs$ageAtRetirement1[hrs$earliestRetire <= 4 & hrs$persons == 1] ) 
  ...
  hrs$cndurImputed[hrs$earliestRetire < 5 & hrs$persons == 2] <-  (fit4$coefficients[1] + fit4$coefficients[2] * hrs$ssIncAtRetire1[hrs$earliestRetire <= 4 & hrs$persons == 2] / hrs$gtw[hrs$earliestRetire <= 4 & hrs$persons == 2] + fit4$coefficients[3] * hrs$ssIncAtRetire1[hrs$earliestRetire <= 4 & hrs$persons == 2] / hrs$gtw[hrs$earliestRetire <= 4 & hrs$persons == 2] + fit4$coefficients[4] * hrs$portfolioAssetsTotal[hrs$earliestRetire <= 4 & hrs$persons == 2] / hrs$gtw[hrs$earliestRetire <= 4 & hrs$persons == 2] + fit4$coefficients[5] * hrs$pensionIncome[hrs$earliestRetire <= 4 & hrs$persons == 2] / hrs$gtw[hrs$earliestRetire <= 4 & hrs$persons == 2] + fit4$coefficients[6] * hrs$ageAtRetirement1[hrs$earliestRetire <= 4 & hrs$persons == 2] + fit4$coefficients[7] * hrs$ageAtRetirement2[hrs$earliestRetire <= 4 & hrs$persons == 2]) 
  ...
  write.csv(xlSummary,"Imputed2 CAMS.csv")  # write a summary to Excel .csv file
  
  # create column of normalizing denominator (gtw) in hrs data set
  hrs$gtw <- hrs$portfolioAssetsTotal+hrs$paycheck + hrs$pensionIncome + hrs$ssIncAtRetire1 + hrs$ssIncAtRetire2
  
  hrsInCams2na <- hrsInCams2[!is.na(hrsInCams2)]

 #  for (j in 1:length(hrsInCams2na)) {
 #    camsr <- which(cams$hhid == hrsInCams2na[j])
 #    hrsr <- which(hrs$hhid == hrsInCams2na[j])
 #    if (round(cams$predictedCndur[camsr]) != round(hrs$cndurImputed[hrsr]) & hrs$earliestRetire[hrsr] > 0) break
 #  }
 # 
 # if (j < length(hrsInCams2na)) cat("\n*** Predicted Cndur's not equal for ", hrsInCams2[j] )
 # 