...
# The purpose of this script is to impute CAMS spending data for HRS repondents not included in CAMS.
  ...
  cams <- read.csv('~/Dropbox/Robo Research/Highlighted CAMS Data Set.csv')
  ...
  camCols <- grep("h.*cndur$",colnames(cams))
  cams2 <- cams[,c(1,2,camCols)]  # preserve original CAMS data
  ...
  # delete any CAMS records for households not in our HRD data set
  ...
  # cams2 <- cams2[cams2$earliestRetire >= 5,]
  # afterW5 <- dim(cams2)[1]
  
  # cams2 <- cams[cams$earliestRetire >= 5,]  # no CAMS data for retirement prior to wave 5
  
  # cams2$earliestRetire <- rep(0,dim(cams2)[1])
  
  cams2InHRS <- cams2$hhid[cams2$hhid %in% hrs$hhid]
  for (camsHHID in cams2InHRS) {
    hrsRow <- which(hrs$hhid == camsHHID) 
    camsRow <- which(cams2InHRS == camsHHID)
    cams2$earliestRetire[camsRow] <- hrs$earliestRetire[hrsRow]
    cams2$persons[camsRow] <- hrs$persons[hrsRow]
    cams2$ageAtRetirement1[camsRow] <- hrs$ageAtRetirement1 [hrsRow]
    cams2$ageAtRetirement2[camsRow] <- hrs$ageAtRetirement2 [hrsRow]
    cams2$male[camsRow] <- 1
    if(hrs$gender1[hrsRow] == "2.female") cams2$male[camsRow] <- 0
    cams2$ssIncAtRetire1[camsRow] <- hrs$ssIncAtRetire1[hrsRow]
    cams2$ssIncAtRetire2[camsRow] <- hrs$ssIncAtRetire2[hrsRow]
    cams2$debt[camsRow] <- hrs$debt[hrsRow]
    cams2$portfolioAssetsTotal[camsRow] <- hrs$portfolioAssetsTotal[hrsRow]
    cams2$paycheck[camsRow] <- hrs$paycheck[hrsRow]
    cams2$pensionIncome[camsRow] <- hrs$pensionIncome[hrsRow]
  }

  cndurCols <- grep("h.*cndur$",colnames(cams2))
  
  cams2$cndurWOR <- rep(0,dim(cams2)[1])
  for (j in 1:dim(cams2)[1]) {    # select consumer non-durable spending for wave of retirement
    if (cams2$earliestRetire[j] >= 5) {
      cams2$cndurWOR[j] <- cams2[j,(cndurCols[cams2$earliestRetire[j] - 4])]
    } else {
      cams2$cndurWOR[j] <- NA
    }
  }
  ...
  ########
  # When cndurWOR is available in cams2 (not NA), add it to HRS data
  ########
  ...
  cams1P$gtw <- cams1P$ssIncAtRetire1 + cams1P$ssIncAtRetire2 + cams1P$pensionIncome + cams1P$paycheck + cams1P$portfolioAssetsTotal
  cams1Pgtw <- data.frame("hhid"=cams1P$hhid,"earliestRetire"=cams1P$earliestRetire,"cndurWOR"=cams1P$cndurWOR,"ageAtRetirement1"=cams1P$ageAtRetirement1,"male"=cams1P$male,"ssIncAtRetire1"=cams1P$ssIncAtRetire1/cams1P$gtw,"debt"=cams1P$debt,"portfolioAssetsTotal"=cams1P$portfolioAssetsTotal/cams1P$gtw,"paycheck"=cams1P$paycheck/cams1P$gtw,"pensionIncome"=cams1P$pensionIncome/cams1P$gtw,"gtw"=cams1P$gtw)
  ...
  # normalizing variable for all CAMS
...
# create columns =1 if retirement in that wave, else zero  
  ...
  cams1Pgtw <-cams1Pgtw[cams1model$earliestRetire >= 5,]
  ...
  cams1Pgtw$waveOfRetirement <- cams1P$earliestRetire
...
# fit2 <- lm(cams2Pgtw$cndurWOR ~ cams2Pgtw$d5 + cams2Pgtw$d6 + cams2Pgtw$d7 + cams2Pgtw$d8 + cams2Pgtw$d9 + cams2Pgtw$d10 + cams2Pgtw$d11 + cams2Pgtw$d12 + cams2Pgtw$ssIncAtRetire1 + cams2Pgtw$ssIncAtRetire2 + cams2Pgtw$portfolioAssetsTotal + cams2Pgtw$pensionIncome + cams2Pgtw$ageAtRetirement1 + cams2Pgtw$ageAtRetirement2 -1) # "-1" tells lm to ignore the y-intercept. d-columns will be the y-intercept.
  ...
  xlSummary$imputed <- rep(FALSE,dim(xlSummary)[1])
  xlSummary$imputed[xlSummary$selectedCndur == xlSummary$predictedCndur] <- TRUE
