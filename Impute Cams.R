...
# predictions with smaller errors. Lrage error predictions are those that exceed 100%.
...
# crossValOnePerson[crossValOnePerson > 5] <- 0
# plot(density(crossValOnePerson[,1]),main="Density Plots for 1-Person Prediction Errors\nabsolute error as fraction of reported consumption",ylim=c(0,2),xlim=c(0,4))
# for (i in 2:n){
#   lines(density(crossValOnePerson[,i]),ylim=c(0,2),xlim=c(0,4))
# }
# 
# # graph density of all n scenarios for one-person households
# cat("\nRemoved ",sum(crossValTwoPerson > 5)," outliers with greater than 500% error from ",length(crossValTwoPerson)," samples.")
# crossValTwoPerson[crossValTwoPerson > 5] <- 0
# plot(density(crossValTwoPerson[,1]),main="Density Plots for 2-Person Prediction Errors\nabsolute error as fraction of reported consumption",ylim=c(0,2),xlim=c(0,4))
# for (i in 2:n){
#   lines(density(crossValTwoPerson[,i]),ylim=c(0,2),xlim=c(0,4))
# }

# hrs2P <- hrs[hrs$persons == 2,]
