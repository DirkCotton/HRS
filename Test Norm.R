...
# The purpose of this script is to impute CAMS spending data for all HRS repondents using dummy variables for wave of retirement as the constant for cHat.
 ...
 # normalize actual consumption data
  ...
  ###Normalize actual consumtion data
  cams1Pgtw$cndurNorm <- cams1Pgtw$cndurWOR /  cams1Pgtw$gtw
...
# create columns =1 if retirement in that wave, else zero  
  ...
  hrs$cndurNorm <- rep(NA,dim(hrs)[1])
  hrs$cndurNorm <- hrs$cndurWOR / hrs$gtw
  
  fit <- lm(cams1Pgtw$cndurNorm ~ 
              cams1Pgtw$d5 
            + cams1Pgtw$d6 
            + cams1Pgtw$d7 
            + cams1Pgtw$d8 
  ...
  hrs$cndurImputed[hrs$earliestRetire >= 5 & hrs$persons == 1] <- 
   ...
   hrs$cndurImputed[hrs$earliestRetire >= 5 & hrs$persons == 1] <- hrs$cndurImputed[hrs$earliestRetire >= 5 & hrs$persons == 1] * hrs$gtw[hrs$earliestRetire >= 5 & hrs$persons == 1]
...
#########################################  
  ...
  cams2Pgtw$male <- NA
  ###Normalize actual consumtion data
  cams2Pgtw$cndurNorm <- cams2Pgtw$cndurWOR /  cams2Pgtw$gtw
  ...
  hrs$cndurNorm[hrs$earliestRetire >= 5 & hrs$persons == 2] <- hrs$cndurImputed[hrs$earliestRetire >= 5 & hrs$persons == 2] * hrs$gtw[hrs$earliestRetire >= 5 & hrs$persons == 2]
  ...
  hrs$cndurNorm[hrs$earliestRetire <= 4 & hrs$persons == 1] <- hrs$cndurImputed[hrs$earliestRetire <= 4 & hrs$persons == 1] * hrs$gtw[hrs$earliestRetire <= 4 & hrs$persons == 1]
  ...
  hrs$cndurImputed[hrs$earliestRetire < 5 & hrs$persons == 2] <- hrs$cndurImputed[hrs$earliestRetire < 5 & hrs$persons == 2] * hrs$gtw[hrs$earliestRetire < 5 & hrs$persons == 2]
  
  # eliminate NA predictions when gtw == 0
  hrs$cndurNorm[hrs$gtw == 0] <- 0
  ...
  hrs$cndurImputed[4] <- 
    (  fit$coefficients[1]  * hrs$d5[4] 
       + fit$coefficients[2]  * hrs$d6[4] 
       + fit$coefficients[3]  * hrs$d7[4] 
       + fit$coefficients[4]  * hrs$d8[4] 
       + fit$coefficients[5]  * hrs$d9[4] 
       + fit$coefficients[6]  * hrs$d10[4] 
       + fit$coefficients[7]  * hrs$d11[4] 
       + fit$coefficients[8]  * hrs$d12[4] 
       + fit$coefficients[9]  * hrs$male[4] 
       + fit$coefficients[10] * hrs$ssIncAtRetire1[4] / hrs$gtw[4] 
       + fit$coefficients[11] * hrs$portfolioAssetsTotal[4] / hrs$gtw[4] 
       + fit$coefficients[12] * hrs$pensionIncome[4] / hrs$gtw[4] 
       + fit$coefficients[13] * hrs$ageAtRetirement1[4] 
    )   
  
  hrs$cndurImputed[4] <- hrs$cndurImputed[4] * hrs$gtw[4]
